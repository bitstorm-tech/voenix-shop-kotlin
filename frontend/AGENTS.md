...
- The admin article surface is per article type all the way up, mirroring the per-type backend routes: one list page (`MugArticlesView`, `TshirtArticlesView`), one store (`stores/admin/mugArticles.ts`, `stores/admin/tshirtArticles.ts`), and one editor per type. There is no merged "all articles" page and no `articleType` discriminator in the admin stores; a new article type gets its own list view, store, editor, and sidebar entry. What every type shares (the list row shape, the error classes, the pre-upload helper) lives in `stores/admin/articles.ts`.
- The two route-level article editors (`MugArticleEditView`, `TshirtArticleEditView`) stay separate views, one per article type. Their shared lifecycle — load by route id, create versus update, not-found handling, the price gate, delete — lives in `composables/useAdminArticleEditor.ts`; what differs (form state, rules, payload, the type's store, the list route to return to) is passed in as closures.
