...
describe('ConfirmEmailView', () => {
  ...
  it('shows the link-specific copy for an INVALID_LINK 400', async () => {
    stubEndpoint('/api/auth/confirm-email', () => jsonResponse(invalidLinkBody, 400))

    const wrapper = await mountAt(ConfirmEmailView, '/confirm?userId=1&token=stale')

    expect(wrapper.findComponent(AuthStatus).props('message')).toBe(
      'auth.confirmEmail.error.invalidLink',
    )
  })

  it('keeps the generic copy for a codeless failure', async () => {
    stubEndpoint('/api/auth/confirm-email', () =>
      jsonResponse({ message: 'Internal server error', errors: {} }, 500),
    )

    const wrapper = await mountAt(ConfirmEmailView, '/confirm?userId=1&token=stale')

    expect(wrapper.findComponent(AuthStatus).props('message')).toBe(
      'auth.confirmEmail.error.message',
    )
  })
})
