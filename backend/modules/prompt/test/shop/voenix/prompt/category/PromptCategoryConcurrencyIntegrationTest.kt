...
/**
 * Whether the category display order survives concurrent writers.
 *
 * Every writer that changes a position first locks the `CATEGORY` anchor row of `prompt_ordering`,
 * so the tests below describe what that lock buys: two reorders queue instead of interleaving, a
 * create cannot reuse a position a reorder is about to write, and only a writer that ignores the
 * lock can still leave the sequence gapped or make the reorder lose the deferred unique check at
 * COMMIT. Both of those answer the same retryable conflict, and both leave the stored order exactly
 * as they found it.
 */
internal class PromptCategoryConcurrencyIntegrationTest : PostgresIntegrationTest() {
    @Test
    fun `two concurrent reorders serialize and leave a dense sequence`() {
        migratedDataSource("prompt-category-concurrent-reorder-test").use { dataSource ->
            ...
            runBlocking {
                ...
                results.forEach { result ->
                    val order = assertIs<OperationResult.Success<List<PromptCategory>>>(result)
                    assertEquals(listOf(1, 2, 3, 4), order.value.map(PromptCategory::position))
                }
            }
    ...
    fun `a create running next to a reorder cannot corrupt the sequence`() {
        migratedDataSource("prompt-category-concurrent-create-test").use { dataSource ->
            PromptTestSchema.reset(dataSource)
            PromptTestSchema.seedCategories(dataSource, "First", "Second", "Third", "Fourth")
            val categories = categoryService(dataSource)
            ...
            runBlocking {
                val results = coroutineScope {
                    listOf(
                            async(Dispatchers.IO) {
                                categories.create(PromptCategoryInput(name = "Fifth"))
                            },
                            async(Dispatchers.IO) {
                                categories.reorder(ReorderInput(sourceId = 4, targetId = 1))
                            },
    ...
    fun `concurrent case-variant creates leave one row and one conflict`() {
        migratedDataSource("prompt-category-concurrent-name-test").use { dataSource ->
            PromptTestSchema.reset(dataSource)
            val categories = categoryService(dataSource)
            ...
            runBlocking {
                val results = coroutineScope {
                    listOf(
                            async(Dispatchers.IO) {
                                categories.create(PromptCategoryInput(name = "Race"))
                            },
                            async(Dispatchers.IO) {
                                categories.create(PromptCategoryInput(name = "RACE"))
                            },
            ...
            assertEquals(1, PromptTestSchema.orderedCategories(dataSource).size)
        }
    }

    @Test
    fun `a gap in the stored sequence is refused before anything is written`() {
        migratedDataSource("prompt-category-gapped-order-test").use { dataSource ->
            PromptTestSchema.reset(dataSource)
            PromptTestSchema.seedCategories(dataSource, "First", "Second", "Third")
            // A writer that ignored the ordering lock — a manual fix, for instance — left a gap.
            PromptTestSchema.execute(
                dataSource,
                "UPDATE voenix.prompt_categories SET position = 5 WHERE position = 3",
            )
            val categories = categoryService(dataSource)
    ...
    fun `a position written outside the ordering lock makes the reorder fail at commit`() {
        migratedDataSource("prompt-category-commit-conflict-test").use { dataSource ->
            PromptTestSchema.reset(dataSource)
            PromptTestSchema.seedCategories(dataSource, "First", "Second", "Third", "Fourth")
            val categories = categoryService(dataSource)

            dataSource("prompt-category-commit-conflict-raw").use { rawSource ->
                rawSource.connection.use { raw ->
                    ...
                    raw.createStatement().use { statement ->
                        statement.execute(
                            ...
                            UPDATE voenix.prompt_categories SET position = 3 WHERE id = 2;
                            UPDATE voenix.prompt_categories SET position = 4 WHERE id = 3;
                            UPDATE voenix.prompt_categories SET position = 1 WHERE id = 4;
                            """
                                .trimIndent()
                        )
                    }
    ...
    private fun categoryService(dataSource: DataSource): PromptCategoryService =
        PromptCategoryService(PromptCategoryRepository(Database.connect(datasource = dataSource)))
    ...
    private fun assertDenseOrder(
        ...
        val stored = PromptTestSchema.orderedCategories(dataSource)
