 ...
 *
 * This is what turns "no N+1" from a claim into a measurement: the statements a list of one prompt
...
internal class CountingDataSource(private val delegate: DataSource) : DataSource by delegate {
     ...
     * prompt into the same query turns `= ?` into `IN (?, ?)`, and that is exactly the difference
