 ...
 *
 * It is registered outside the `authenticate` block of [installPromptRoutes], which is the whole
 * point of a separate installer: a customer choosing a prompt has no session, so anonymous access
 * is not a rule this handler applies but the absence of the admin subtree around it. The path is
 * `/api/prompts` rather than `/api/admin/prompts`, so the two trees cannot be confused by a reader
 * or by Ktor.
 *
 ...
 */
internal fun Application.installPublicPromptRoutes(prompts: PublicPromptOperations) {
    routing {
        get(PATH) {
            val requested =
            ...
            when (val result = prompts.list(categoryId)) {
                ...
                else -> error("A public prompt read has no outcome besides success and a failure")
            }
        }
    }
}
