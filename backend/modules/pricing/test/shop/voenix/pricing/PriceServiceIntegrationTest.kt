...
internal class PriceServiceIntegrationTest : PostgresIntegrationTest() {
    ...
    @Test
    fun `calculation resolves distinct vats in one batch lookup`() = runBlocking {
        migratedDataSource("pricing-vat-batch-test").use { dataSource ->
            resetPricing(dataSource)
            val database = Database.connect(datasource = dataSource)
            val reader = CountingVatReader(createVatReader(database))
            val service = PriceService(PriceRepository(database), reader)

            assertIs<OperationResult.Success<CalculatedPrice>>(
                service.calculate(validInput().copy(purchaseVatId = 1, salesVatId = 2))
            )
    ...
    fun `database failures are hidden behind unexpected failure results`() = runBlocking {
        ...
        resetPricing(dataSource)
        val database = Database.connect(datasource = dataSource)
        val service = PriceService(PriceRepository(database), createVatReader(database))
        dataSource.close()
