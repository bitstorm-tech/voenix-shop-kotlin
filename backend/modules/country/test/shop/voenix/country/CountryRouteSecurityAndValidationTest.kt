...
internal class CountryRouteSecurityAndValidationTest {
    @Test
    ...
    fun `binding failures return generic api errors without calling country operations`() =
        testApplication {
            ...
            val unsupported =
                listOf(
                    admin.post("/api/admin/countries") { header(AuthRouting.CSRF_HEADER, token) },
                    admin.post("/api/admin/countries") {
                        header(AuthRouting.CSRF_HEADER, token)
    ...
    fun `country results map to small api errors`() = testApplication {
        ...
        assertApiError(
            admin.delete("/api/admin/countries/999") { header(AuthRouting.CSRF_HEADER, token) },
            HttpStatusCode.NotFound,
            "Country not found",
        )
