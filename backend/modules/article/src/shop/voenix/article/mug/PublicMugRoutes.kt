 ...
 *
 * They are registered outside the `authenticate` block of [MugArticleRoutes], which is the whole
 * point of a separate object: a customer browsing the shop has no session, so anonymous access is
 * not a rule these handlers apply but the absence of the admin subtree around them. The paths are
 * `/api/articles/...` rather than `/api/admin/articles/...`, so the two trees cannot be confused by
 * a reader or by Ktor.
 *
 ...
 */
internal object PublicMugRoutes {
    private const val BASE_PATH = "/api/articles/mugs"

    fun install(
        application: Application,
        mugs: PublicMugOperations,
    ) {
        application.routing {
