...
(`.js`, `.css`, `.woff2`) may be cached for a year (`immutable`), because a
new build produces new file names, never new content under an old name.
...
described in
[`running-the-development-server.md`](running-the-development-server.md).
...
1. Frontend build (`oven/bun`): `bun install`, then `bun run build`, which
   type-checks with vue-tsc and bundles with Vite into `dist/`.
...
| `MOLLIE_API_KEY`, `MOLLIE_REDIRECT_URL`, `MOLLIE_WEBHOOK_URL`, `MOLLIE_WEBHOOK_SECRET` | the payment settings, see [`payment-package.md`](../backend/packages/payment-package.md) |
...
configuration layering was designed for. Unlike the environment layer,
...
- A secret file named `application-production.yaml` with the full
  configuration. The template is below.
- A Docker Command override (in the service's *Settings*), because the
  ...
  `application-container.yaml` is deliberately *not* in this command.
...
A disk makes the service single-instance (no horizontal scaling), and
zero-downtime deploys are unavailable with a disk attached. Both are fine for
...
defaults, and an empty string `""` would override with emptiness, so never write
`""`. `frontend.distPath` stays bare, because `application-fullstack.yaml`
already sets it.
