...
Mind the order inside the route: the CSRF protection is installed first. A
...
at least `1`, because a `Retry-After: 0` would only invite an immediate retry. A
refused request never pushes the window further away, so the wait always
shrinks.
...
all traffic would be counted as a single client. That is annoying, but it fails
...
The day the backend is scaled out, the state has to move somewhere shared, such
as a Redis counter or a database table, and `ClientIpRateLimiter` is the one
class that has to change. Nothing else knows how the counting works.
