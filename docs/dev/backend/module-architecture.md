...
| --- | --- | --- |
| `platform` | none | Authentication, database startup, the `Database.read` and `Database.write` transaction helpers every repository runs its queries in (see [Persistence error handling](persistence-error-handling.md)), HTTP runtime, the exported `readChunks` every handler that streams a request body itself reads with, so a body cut off by the request size limit cannot pass for a complete one (see [Request size limits](request-size-limits.md)), the per-IP rate limit a route can install (see [Rate limiting](rate-limiting.md)), the validation bridge and the shared `ValidationErrorsBuilder` every request type collects its field errors with (see [Request validation](request-validation.md)), and shared operation results |
