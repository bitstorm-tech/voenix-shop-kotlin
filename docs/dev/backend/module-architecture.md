...
- `EmailModule` exports only `UserEmailSender` and `EmailOutbox`; the app-owned
  ...
  modules that resolve queued references. Two branches cover three mail kinds:
...
6. create the app-owned `LateBoundProductionSource` and `LateBoundPaymentStatus`
   ...
   mail kinds, bound into the aggregated queued source. Only
...
11. install Checkout, the last consumer in the chain, with the six capabilities
    ...
    `order.placement`, `order.payments`, `payments.starter`, and the catalog's
    `shippableCountries`, plus `GuestTokens`. Checkout is the second consumer
    of `order.payments`, for the free-order `confirm` alone. It returns nothing,
...
13. install Generator with its settings, the catalog's `PromptCatalog`,
    MagicCoins' `GenerationCoins`, and the same `GuestTokens`. Generator is the
    second consumer of the prompt catalog and the only consumer of the coin
    capability.
...
moves that guest's balance from 10 to 9 in the real database. That only works when the prompt catalog and
