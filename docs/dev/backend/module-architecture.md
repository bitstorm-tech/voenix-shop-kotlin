...
## Package, Kotlin module, and Ktor module

These three similar terms describe different boundaries:
...
- A **Ktor module** is a runtime function such as `Application.module()` that
  installs plugins and routes into a running server. Calling several feature
...
because `platform` owns database startup and migration. Feature packages keep
...
A module should expose a small capability, not its object graph. Feature table
objects, repositories, services, routes, row mappings, and persistence result
types are `internal`. This prevents another feature from bypassing the owning
...
- each feature exposes an `install...Feature` function for Ktor composition;
- each feature exposes a `validate...Requests` function so `app` can install
...
The application does not construct or import a feature's repository, service,
or routes. Each feature factory assembles those internal details itself.
...
chain. Feature and app modules depend on `test-support` only through
...
When a new feature needs its own compile-time boundary:
...
consumer needs behavior from another feature, add the narrowest useful reader
