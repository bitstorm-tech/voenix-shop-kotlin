...
[Kotlin source file organization](../conventions/source-file-organization.md) describes it:
...
- **A broken balance lookup is never an empty balance.**
  ...
  [`OperationResult`](../conventions/operation-results.md), not a `Boolean`, so that a
...
[Magic Coins package guide](magic-coins-package.md) and
...
- **Both provider answers are treated as hostile.** The result URL must be
  ...
  stored and paid for. See [Request size limits](../conventions/request-size-limits.md).
...
is in [Rate limiting](../conventions/rate-limiting.md).
...
See [Backend compile-time modules](../conventions/module-architecture.md) for where the call
