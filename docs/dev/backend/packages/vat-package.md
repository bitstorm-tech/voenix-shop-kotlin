...
- VAT entries are admin-managed master data behind `/api/admin/vat`; there is
  no public VAT route, and the only thing other modules see is `VatReader`.
- A request runs through `VatRoutes`, the internal `VatOperations` seam,
  `VatService`, and `VatRepository`. `VatInput.validate()` holds every field
  rule once; the Request Validation plugin and the service both call it, so the
  repository only ever receives valid, normalized values.
- The auth-owned `AdminRouteProtection` covers the whole `/api/admin/vat`
  subtree: session, `ADMIN` role, and CSRF are checked before a handler runs.
- At most one entry is the default. Create and update demote the previous
  default and write the new row in one serializable transaction, and the partial
  unique index is the final guarantee.
- PostgreSQL decides conflicts: SQL state `23505` becomes
  `VatWriteResult.Conflict` and a generic `409`, the foreign-key violation on a
  referenced delete becomes `VatDeleteResult.InUse` and `409 VAT is in use`.
  Constraint names are never inspected.
- `installVatModule(database)` returns `VatReader` for Pricing; every other type
  stays internal to the compilation module.
...
[Kotlin source file organization](../conventions/source-file-organization.md).
...
- [`VatRoutes.kt`](../../../../backend/modules/vat/src/shop/voenix/vat/VatRoutes.kt)
  ...
  [Kotlin code quality](../conventions/kotlin-code-quality.md)); only the constants inside are
  private.
...
- The shared [`OperationResult`](../conventions/operation-results.md) lists the expected success and
