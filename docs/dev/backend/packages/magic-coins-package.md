...
[Authentication and authorization](../conventions/authentication-and-authorization.md#the-guest-tokens-lifetime-around-a-login-and-a-logout)).
...
- `hasEnoughForGeneration` answers an
  [`OperationResult`](../conventions/operation-results.md), so a database failure can never
