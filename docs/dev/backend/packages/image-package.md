...
- `UploadedImage` and `receiveUploadedImage` belong to that API too. They read
  ...
  [Request size limits](../conventions/request-size-limits.md)). It started as an
  ...
  - **`413` is not this layer's status.** It belongs to the application-wide
    ...
    [Request size limits](../conventions/request-size-limits.md) describes. Either way the
  ...
  ```
- `ExampleImages` is the shared rule described in
  [The example-image rule](#the-example-image-rule). It is public because
  Article and Prompt construct one by name.
...
[Request size limits](../conventions/request-size-limits.md) describes.
