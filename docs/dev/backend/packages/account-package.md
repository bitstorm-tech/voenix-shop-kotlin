...
[`authentication-and-authorization.md`](../conventions/authentication-and-authorization.md)).
...
file (see
[`source-file-organization.md`](../conventions/source-file-organization.md)). Every file below
...
collaborators, but neither service ever calls the other. The collaborators are
the repository, the mailer, the password hasher, and the token mechanics, which
got a name of their own: `AccountTokenIssuer`.
...
- **Inline `ApiError` responses.** Where `NotFound → 404` is the contract, as
  in the supplier-login list and delete, the routes use the platform
  `OperationResultHttpMapping` like the rest of the backend (see
  [`operation-results.md`](../conventions/operation-results.md)). The `/api/auth` flows
  ...
  possible follow-up, for the whole backend at once rather than for this
  package alone.)
- **`AccountOperations` has exactly 11 methods**, which is Detekt's
...
The three link flows, confirm-email, reset-password, and confirm-change-email,
...
Authenticated endpoints (session required; mutations also require the
`X-XSRF-TOKEN` header, which is stricter than the legacy app and an approved
deviation):
...
Admin endpoints (session with role `ADMIN`; mutations also require the
`X-XSRF-TOKEN` header):

| Method and path | Success | Failure notes |
| --- | --- | --- |
| `POST /api/admin/supplier-logins` | `201` + `Location` + `SupplierLoginView`, invitation mail sent | `400` invalid input **or** unknown supplier (as a `supplierId` field error), `409` e-mail exists, `502` the provider did not accept the invitation but *the login exists*, `500` |
...
`SupplierLoginView` is `{ userId, email, supplierId, createdAt }`, with no
...
stored reset token with a freshly mailed one. It is the same purpose and the
same `POST /api/auth/reset-password` endpoint.
...
- `502` means *the provider*, and only the provider. `AccountMailer` catches
  ...
  one exception becomes a delivery result. Anything else a send can throw, such
  as a rendering failure, a malformed link, or any other bug of ours, is not
  caught there. It reaches the service's `databaseOperation` guard, which logs it and
...
- Best-effort notifications (password changed, e-mail change notice) are
  logged on failure and never fail the operation. That deliberately includes our
...
- `email_confirmed = true`. Nobody ever mails this address a confirmation link,
  and the login refuses unconfirmed addresses, so an unconfirmed supplier login
...
[`persistence-error-handling.md`](../conventions/persistence-error-handling.md)). The
...
`createAccountModule` builds the parts once and hands them to both services:
one repository, one `AccountTokenIssuer`, one mailer, one password hasher.
`AccountModule.install` then installs both route sets,
...
the standard runtime-handle convention ([`module-architecture.md`](../conventions/module-architecture.md)). The handle
...
- `AccountInputValidationTest` and `PasswordHasherTest` are pure unit tests
  for the complete field-rule matrix and the hash encoding.
- `AccountServiceIntegrationTest` runs the service against real PostgreSQL:
  ...
  behavior.
- `AccountRouteSecurityAndValidationTest` proves that rejected requests (no
  session, bad CSRF, invalid bodies) never reach the operation.
- `AccountFlowIntegrationTest` runs full journeys over HTTP; confirmation and
...
- `AccountSchemaIntegrationTest` runs the Flyway migration on an empty database
  and checks its constraints, including the supplier link: it needs an existing
  supplier, it keeps that supplier from being deleted, and it has its partial
  index.
- `SupplierAccountsIntegrationTest` covers the exported capability: the linked
  supplier for a supplier login, `null` for a customer, and `null` for a user
  that does not exist.
- `SupplierLoginRouteSecurityAndValidationTest` tests the admin surface against
...
- `SupplierLoginFlowIntegrationTest` tests the same surface over HTTP against
  real PostgreSQL: invitation → set password → sign in as `SUPPLIER`, the
  duplicate and unknown-supplier refusals, the `502` whose login survives, and
  the delete matrix. The invitation link comes from the recorded mail, never from the
  token table.
