...
- [Why the routes own three separate subtrees](#why-the-routes-own-three-separate-subtrees)
- [Placing an order](#placing-an-order)
  - [Why the confirmation mail hangs on the placement](#why-the-confirmation-mail-hangs-on-the-placement)
- [Reading an order that still has to be paid](#reading-an-order-that-still-has-to-be-paid)
- [Confirming a payment](#confirming-a-payment)
- [Cancelling an order](#cancelling-an-order)
- [When a payment ends without the order](#when-a-payment-ends-without-the-order)
- [The two read paths that are not a customer](#the-two-read-paths-that-are-not-a-customer)
- [The schema](#the-schema)
- [The seven exported capabilities](#the-seven-exported-capabilities)
...
It differs from the packages migrated before it in three ways:
...
[Kotlin source file organization](../conventions/source-file-organization.md). That is why the
...
| `GET /api/admin/orders/{orderId}/production-pdfs` | admin | `200`, an array of `{ "supplierId": 3, "fileName": "ORD-42.pdf" }` | `404`, `409` with a `PRODUCTION_PDF_*` code, `500` |
...
`EXPIRED`, which is Mollie's vocabulary uppercased, or `null` when the order
has no payment at all. That is the case for a free order, or for one whose
checkout was never started. Note the spelling: the payment value `CANCELED`
carries **one** L while the order `status` value `CANCELLED` carries two. That
is deliberate and must not be "fixed". Mollie cancelling a payment and the shop
cancelling an order are two different facts written by two different systems,
and one spelling would make a status string silently valid on the wrong side.
...
written in two steps. First comes the ownership-filtered read; nothing is ever
refreshed for an order the caller does not own. Then, when that refresh
answered `PAID` while the row still said `PENDING`, one more read of the same
order follows. Only then does the answer describe a single moment in time.
Otherwise the repairing response would carry `"status":"PENDING"` next to
`"paymentStatus":"PAID"` and contradict itself.
...
One read, the token lookup, does not use this rule at all, and that is
deliberate. It carries no identity, because the access token *is* the
credential. See [The access token and the permanent
...
- **no protection plugin, no session, no cookie, no CSRF.** The request carries
  ...
  cookie, because following a mail link must not turn the reader into a
  tracked visitor;
- **`Cache-Control: no-store`**, like every other order answer;
- **one `404` for every miss**, whether malformed token, unknown token, or no
  token at all. Never a `400`, never a `403`, because the difference is the
  only feedback a probe could get;
...
Putting the token in the path rather than in a fragment has one accepted
trade-off: it appears in reverse-proxy request logs. That is mitigated by
`no-store`, `Referrer-Policy: no-referrer` on the page, the redacting type, and
no application-side logging of the token.
...
The legacy PDF endpoint was anonymous, because its admin attribute was
commented out, so every order's production data, addresses included, was
readable by order id.
...
1. **Field rules.** `PlaceOrderInput.validate()` is pure. It checks the
   address lengths, the two-letter country code, the e-mail shape, and the
   line bounds. It also checks two rules that are easy to miss: an order needs
   *someone to show itself to* (a guest token or a user id, the same rule as
   the table's owner CHECK), and its money must describe its own lines
   (`subtotal` is the sum of `(price + promptPrice) × quantity`). Nothing in
   the database checks the second one, so the validator does.
...
There is one accepted edge, documented in the plan of issue #110. An order
that is cancelled right after placement, because a payment could not even be
created, still triggers its confirmation mail. The link then shows the real,
cancelled status, which is why the template's wording is payment-neutral. It
confirms that the order *arrived*, never that it was paid.
...
Both successes carry a `PayableOrder` and not the internal `OrderView`. A
`PayableOrder` holds the order id, the total, the contact fields, and the two
stored addresses. A payment has no use for lines or a status, and keeping the
customer's own view `internal` is what stops it leaking into another module.
...
decision. That is why "the order was paid" and "its side effects exist" are one
committed fact, and why no compensation code exists anywhere in the module. The
...
- **`PromotionRefused(reason)` is a paid order.** When the coupon's usage limit
  ...
  Refusing would leave the customer charged and never delivered. That was the
  legacy outcome, where the order stayed `PENDING` forever. The order becomes
  `PAID` without a redemption and the service logs a warning naming the reason
  the promotion module gave (Joe's decision of 2026-07-31, deviation D22).
...
it is the mirror image of the one above, with the same lock, the same shape,
...
promotion, and only on the `PENDING → CANCELLED` transition, so neither early
return above can release anything. The locked row carries the cart the
...
The order row is locked first all the same. That keeps the lock order of this
...
production request already exists. The service logs a warning and answers
`REFUSED`, which the payment module turns into the manual refund case
(deviation D14 of the Payment migration).
...
attempt through `StoredOrder`. Neither carries an ownership predicate, because
a worker is not a customer. That is why that read is the only one in the module
that no route can reach.
...
shipping" the legacy mail computed (deviation D12). Without that change the
first 100 % coupon would have retried forever against the old
...
- **`orders`** holds the cart it was placed from, the guest token and/or user
...
record survives the account. The consequence, that the order becomes visible
to the guest cookie again, is recorded as deviation D25 and is accepted because
no account-deletion feature exists yet.
...
onward after the install. Everything behind them stays `internal`: operations,
service, repository, tables.
...
- **`OrderItemReader.find(orderItemId, userId, guestToken)`** is what the cart
  ...
  ids. The prices are absent on purpose, because a reorder is charged at
  today's catalog price (deviation D13). So is the quantity, because a reorder
  is a normal add of one line, not a replay of the old order.
...
exactly nine columns for a whole list page in one statement: order id,
`created_at`, and the seven shipping-address and recipient-name fields. No e-mail
...
than in payment is the same rule as for `OrderPaymentGateway`. The cart
re-exports this module, so an interface owned by payment would drag the Mollie
integration into every order consumer.
...
read fails with `IllegalStateException` rather than answering `null`. That
`null` is the contracted word for "this order has no payment", and a customer
who just paid must never be told that.

The module's only setting is `frontendBaseUrl`. It carries `frontend.baseUrl`,
read once by `ApplicationSettings` and shared with the account module, which
mails its links over the same host. `createOrderModule` turns it into the
`OrderLinks` the service builds the confirmation mail's link with.
...
- **Payment.** What a payment *does* to an order, the three writes of
  `OrderPaymentGateway`, lives here, and so does the vocabulary an order answer
  carries: `OrderPaymentStatus` and `OrderPaymentStatusSource` are declared
  here so that no consumer of an order ever compiles against the Mollie
  integration. The payment itself is the payment module's: the `payments`
  table, the provider call, the webhook. This module never learns that a
  provider exists.
...
- **`custom_data` and the `SHIPPED` status.** Both were dead in the .NET
  source and were dropped with the migration (deviations D6 and D7). One only
  ever held `{}`, the other was never written.
