...
- [How startup is divided](#how-startup-is-divided)
- [The public auth interface](#the-public-auth-interface)
- [How authentication is installed](#how-authentication-is-installed)
- [The three authentication data classes](#the-three-authentication-data-classes)
  - [`UserSession`: data stored in the auth cookie](#usersession-data-stored-in-the-auth-cookie)
  - [`UserPrincipal`: identity for one request](#userprincipal-identity-for-one-request)
  - [`CsrfSession`: token and owning user](#csrfsession-token-and-owning-user)
- [What happens to the auth cookie](#what-happens-to-the-auth-cookie)
- [How authorization works](#how-authorization-works)
  - [The supplier protection asks a second question](#the-supplier-protection-asks-a-second-question)
- [Which routes require which checks](#which-routes-require-which-checks)
...
- [Session-secret configuration](#session-secret-configuration)
- [Guest identity](#guest-identity)
  - [The guest token's lifetime around a login and a logout](#the-guest-tokens-lifetime-around-a-login-and-a-logout)
- [Adding another protected route](#adding-another-protected-route)
- [Shared HTTP behavior](#shared-http-behavior)
- [Tests that define the behavior](#tests-that-define-the-behavior)
- [File map](#file-map)
  - [Application composition](#application-composition)
  - [Authentication](#authentication)
  - [Shared HTTP runtime](#shared-http-runtime)
- [Summary](#summary)
...
(see [`account-package.md`](../packages/account-package.md)). The session stores the
...
flowchart LR
    ...
    Cookie["Auth module reads and decrypts<br/>voenix.auth"]
...
1. [`installHttpRuntime()`](../../../../backend/modules/platform/src/shop/voenix/http/HttpRuntime.kt)
   installs application-wide JSON Content Negotiation and `StatusPages`.
2. [`installAuthModule(settings)`](../../../../backend/modules/platform/src/shop/voenix/auth/AuthModule.kt)
...
- `installAuthModule(settings)` installs all of that into the receiving Ktor
  application;
...
All four protections share the same fail-closed core, the `failClosedPlugin`
function in
...
The `installAuthModule` function adds four pieces of behavior:
...
lives next to the auth installation that creates it, and it exists only after
Ktor has accepted the session:
...
declared in the same file as the antiforgery endpoint that mints it, stores:
...
and the auth module perform these steps:
...
capability lives next to the auth installation and provides that identity:
...
- **Carts.** A cart is identified by the user id when it was created signed in
  ...
  [cart package guide](../packages/cart-package.md#who-a-cart-belongs-to)).
...
[Magic Coins package](../packages/magic-coins-package.md#no-balance-merge-when-a-guest-signs-in)).
...
[`installHttpRuntime()`](../../../../backend/modules/platform/src/shop/voenix/http/HttpRuntime.kt)
installs two application-wide Ktor plugins:
...
The HTTP runtime does not change Ktor path matching. Product and auth modules use
...
- [`Application.kt`](../../../../backend/app/src/shop/voenix/Application.kt) loads
  settings and installs the HTTP runtime, the auth module, and the product
  modules as separate concerns.
