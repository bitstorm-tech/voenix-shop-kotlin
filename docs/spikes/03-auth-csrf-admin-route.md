# Spike: Auth, CSRF, and Admin Route

## Result

The Kotlin port now has a proof-only Ktor auth route set backed by Postgres:

- `POST /spike-auth/login` validates email/password against `spike_auth_users`, requires confirmed email, sets a signed cookie session, and returns an `X-CSRF-Token` header.
- `POST /spike-auth/logout` clears the cookie session.
- `GET /spike-admin/proof` requires an authenticated admin user.
- `POST /spike-admin/proof` requires authenticated admin role plus matching `X-CSRF-Token`.
...
- Use a vetted password hashing library for production, preferably Argon2id or bcrypt, not the spike PBKDF2 helper.
...
- CSRF delivery: header from login is enough for the spike; real UI should use a consistent hidden-field/header pattern.
